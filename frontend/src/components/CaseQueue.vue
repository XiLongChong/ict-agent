...
const filtered = computed(() => (props.cases || []).filter((item) => (!type.value || item.case_type === type.value) && (!status.value || item.status === status.value)));
</script>
...
<template>
  <div class="view-stack">
    ...
    <v-card class="flat-panel queue-panel">
      <div class="filter-bar"><v-select v-model="type" :items="typeOptions" label="案件类型" /><v-select v-model="status" :items="statusOptions" label="案件状态" /><span>共 {{ filtered.length }} 个案件</span></div>
