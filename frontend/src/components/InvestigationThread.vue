  ...
  <section class="mx-auto w-full max-w-[1280px] space-y-4 pb-10" aria-live="polite">
      ...
      <div class="divide-y divide-border px-5">
        <article v-for="event in events" :key="event.sequence" class="py-4">
          <div class="flex items-center gap-2">
            ...
            />
            <strong class="text-[13px] text-ink">
              {{ event.tool_name ? labels.tool[event.tool_name] : labels.event[event.event_type] }}
            </strong>
          </div>
          <p class="mt-1 pl-6 text-[13px] leading-6 text-muted">{{ event.message }}</p>
          <details v-if="event.evidence?.arguments || event.arguments" class="ml-6 mt-2">
            ...
            <div class="mt-2 rounded-lg bg-canvas p-3">
              ...
              <template v-if="event.evidence">
                <p class="mt-2 text-[13px] leading-6 text-ink">{{ event.evidence.summary }}</p>
    ...
    <template v-if="record">
      <article class="card overflow-hidden">
        ...
        <div class="p-5">
          ...
          <div class="mt-6 border-t border-border pt-5">
            ...
            <ol class="mt-3 space-y-3">
              <li
                ...
                :key="item"
                class="grid grid-cols-[24px_minmax(0,1fr)] gap-2 text-[13px] leading-6 text-muted"
              >
          ...
          <p class="mt-5 rounded-lg bg-warning-wash px-4 py-3 text-[13px] leading-6 text-warning-deep">
      ...
      <details class="card">
        ...
        <div class="space-y-6 border-t border-border px-5 py-5">
          <section>
            ...
            <p class="mt-2 text-[13px] leading-6 text-muted">{{ record.report.investigation_summary }}</p>
          </section>
          ...
          <section>
            ...
            <div class="mt-3 grid grid-cols-1 gap-4 md:grid-cols-3">
              <div>
                ...
                <ul class="mt-1 space-y-1 text-[13px] leading-6 text-muted">
            ...
            <div class="mt-3 space-y-3">
              ...
              >
                <div class="flex items-start gap-2">
                  ...
                  <strong class="text-[13px] leading-6 text-ink">{{ item.statement }}</strong>
                </div>
          ...
          <section>
            ...
            <ul class="mt-2 space-y-1 text-[13px] leading-6 text-muted">
          ...
          <section v-if="record.report.trace?.length">
            ...
            <div class="mt-2 divide-y divide-border">
              <article v-for="item in record.report.trace" :key="item.created_at + item.title" class="py-3">
                <div class="flex items-center gap-2">
                  <strong class="text-[13px] text-ink">{{ item.title }}</strong>
            ...
            <div class="mt-2 divide-y divide-border">
              <article v-for="item in record.evidence" :key="item.evidence_id" class="py-3">
                <div class="flex flex-wrap items-center gap-2">
                  <strong class="text-[13px] text-ink">{{ labels.tool[item.tool_name] }}</strong>
    ...
    <div v-if="error" class="flex items-start gap-3 rounded-lg border border-danger/30 bg-danger-wash p-4">
      ...
      <div>
        ...
        <p class="mt-1 text-[13px] text-danger-deep">{{ error }}</p>
      </div>
    </div>
  </section>
</template>
