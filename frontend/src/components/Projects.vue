...
<template>
  <div class="space-y-5">
    <div class="flex flex-wrap items-center gap-3">
      <h2 class="text-[1.0625rem] font-bold text-ink">项目评估</h2>
      <Badge tone="warning">模拟数据</Badge>
      <p class="text-[0.8125rem] text-muted">对拟立项的模拟新项目执行事前评估（黑名单拦截 / 金额档位 / 历史超期 / 担保人）</p>
    </div>
    ...
    <section class="grid grid-cols-1 gap-4 xl:grid-cols-2">
      <div v-for="item in newProjects" :key="item.project_id" class="card p-5">
        <div class="flex items-start justify-between gap-3">
          <div>
            <strong class="block text-[0.9375rem] text-ink">{{ item.name }}</strong>
        ...
        <div class="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-[0.8125rem] text-muted">
    ...
    <Modal :open="Boolean(assessment)" title="事前评估结论" @close="assessment = null">
      <div v-if="assessment" class="space-y-4">
        ...
        <p class="text-[0.8125rem] text-muted">{{ assessment.project_id }} · 金额 {{ formatAmountTier(assessment.amount_wan) }} · 档位 {{ formatAmountTier(assessment.amount_tier) }}</p>
        ...
        <p class="text-[0.75rem] text-muted">评估时间：{{ assessment.evaluated_at }} · 模拟数据</p>
      </div>
    </Modal>
  </div>
</template>
