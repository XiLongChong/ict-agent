...
const cards = computed(() => {
  ...
  return [
    ["累计销售额", formatMoney(overview["销售额"]), "含退货负值", "#4285f4"],
    ["累计回款额", formatMoney(overview["回款额"]), "全数据窗口", "#34a853"],
    ["最新应收余额", formatMoney(ar["应收余额"]), props.data.latest_ar.period, "#fbbc05"],
    ["最新超期率", formatPercent(ar["超期率"]), `超期 ${formatMoney(ar["超期应收"])}`, "#ea4335"],
  ];
});
...
<template>
  <div class="view-stack">
    ...
    <div class="metric-grid"><v-card v-for="card in cards" :key="card[0]" class="metric-card" :loading="loading"><span class="metric-accent" :style="{ background: card[3] }"></span><span>{{ card[0] }}</span><strong class="compact">{{ card[1] }}</strong><small>{{ card[2] }}</small></v-card></div>
