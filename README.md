...
```text
企业预警 / 规则扫描 / 事前交易 → 统一案件 → AI 证据调查 → 人工复核 → 飞书协同
```
...
- **事前交易**：按客户同业务类型历史订单生成正常、临界或异常新交易，并创建统一调查案件；不改真实数据。
- **案件处理**：集中查看案件概况、来源信号、AI 审查报告和人工复核记录。
- **AI 审查**：根据案件类型查询拟交易、业务画像、应收、销售回款、合同、授信、展期或库存证据，并生成可追溯的结论和处理建议；页面可展开查看最后一次完整模型请求与最终响应 JSON。
...
AI 只能调用系统注册的只读证据工具，不能执行任意 SQL、Python、Shell、文件访问或联网搜索。页面默认展示查询过程、证据和经过校验的报告，并在开发调试区原样展示最后一次累计请求与最终响应，包括 DeepSeek 返回的 `reasoning_content`。
