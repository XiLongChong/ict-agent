...
- **AI 审查**：按案件类型和全部命中信号的证据策略查询拟交易、业务画像、应收、退货、回款冲销、超长账龄、合同毛利/账期、授信、展期或库存超期证据；每次查询披露总行数、实际行数和截断状态，再生成决策优先的结论、未校准概率区间、关键数据冲突和结构化处理建议。页面按需加载最后一次 DeepSeek Chat Completions HTTP 请求和响应摘要，并可直接下载完整事务 JSON。
- **人工复核**：支持确认风险、要求补充证据和确认无风险三类结论。
- **飞书协同**：飞书网页应用免注册登录，按 `open_id` 自动建档；AI 审查和人工复核记录实际操作者，完成/中断结果私聊通知发起人；通知群作为可选的团队副本继续保留。
...
飞书接入为可选能力。两项飞书配置同时填写后，服务会建立官方长连接。用户私聊机器人或从飞书网页
应用进入时，系统按当前应用下唯一的 `open_id` 自动建立用户记录，不需要用户名、密码、手机号或通讯录
权限。从飞书发起的 AI 审查和人工复核会记录姓名与身份类型，调查完成或中断后由机器人私聊通知发起人。

团队通知群不再是个人使用前提。需要团队副本时，把机器人加入群聊并发送“@机器人 绑定通知群”；
群绑定信息保存在案件库中，服务重启不会丢失。主动通知通过官方消息 API 发送，不依赖长连接是否短暂
断开。公开服务器网页继续允许完整演示，未从飞书进入的操作明确记录为“网页访客”。
...
```dotenv
ICT_PUBLIC_BASE_URL=https://risk.example.com
```

然后在飞书开发者后台为现有“佳华智审”应用完成以下配置并发布新版本：

- 应用能力 → 网页应用：桌面端和移动端主页填写
  `https://risk.example.com/api/v1/auth/feishu/start`
- 安全设置 → 重定向 URL：填写
  `https://risk.example.com/api/v1/auth/feishu/callback`
- 可用范围：加入实际使用者；未知的外部评委继续通过公开服务器网址使用
- 机器人权限：保留“以应用身份发消息”、接收单聊消息，以及需要群绑定时的群聊 @ 消息权限

OAuth 登录和获取基础用户信息不需要新增通讯录、手机号或邮箱权限。网页应用内打开时，飞书验证用户后
返回 `open_id`、姓名和 `tenant_key`；后端只保存这些最小身份字段，并签发 7 天 HttpOnly 同源会话。
...
日常更新代码只需要跑仓库自带的一键脚本（会拉代码、重建镜像、更新容器并等待健康检查）：

```bash
sh deploy.sh
```

数据库需要重建时（比如代码带数据库结构变更、或要把旧案件全部清掉重建），用：

```bash
sh rebuild.sh ict-agent
```

它会停止容器、删除两个 DuckDB 数据库，再启动容器让 entrypoint 自动重新导入 CSV 并重新扫描案件。
**会清空全部案件、调查报告、人工审核与模拟交易记录。**

另外，代码里带数据库结构/格式变更时，可以直接调高 `docker-compose.yml` 里的
`ICT_DATA_FORMAT_VERSION`（或 `.env` 中同名变量）再 `sh deploy.sh`：容器启动时会检测版本标记
不匹配，自动删除旧库、重新导入并重新扫描，不需要手动 `sh rebuild.sh`。这个版本号平时不用动。
...
- 公开服务器网页不要求登录，任何拿到演示地址的人仍可启动调查和提交复核；此类操作记录为网页访客。
- 飞书网页应用提供身份识别和个人通知，不把企业自建应用包装成面向任意外部租户的应用商店产品。
- 当前不做角色、部门或数据权限控制；飞书身份只用于操作者审计和通知路由。
