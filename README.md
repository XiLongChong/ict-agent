...
- **AI 审查**：按案件类型和全部命中信号的证据策略查询拟交易、业务画像、应收、退货、回款冲销、超长账龄、合同毛利/账期、授信、展期或库存超期证据；每次查询披露总行数、实际行数和截断状态，再生成可追溯的结论和处理建议。页面按需加载最后一次 DeepSeek Chat Completions HTTP 请求和响应摘要，并可直接下载完整事务 JSON。
