...
- 可能性判断没有支持证据，或引用了不存在的证据编号；
- 同一个证据编号同时出现在同一可能性判断的支持与反驳列表；
- 未校准概率区间上下限非法，或存在关键缺失证据时仍给出过窄区间；
- 在缺少相应数据时直接断言已确认坏账、一定不可回收、停供、无回款能力或已进入诉讼程序。

模型只生成业务判断字段：决策摘要、风险信号、工具事实、未校准概率区间、关键数据冲突、限制和结构化
行动。`report_schema_version`、`evidence_completeness`、`requires_human_review` 与真实执行轨迹由后端
确定性补入，不进入模型输出 Schema，避免模型生成随后被覆盖的审计字段。

报告展示风险信号判断、工具直接证明的事实、带证据和概率区间的可能性、关键数据冲突与人工行动。
...
最低覆盖完成时保留规则与证据共同支持的早期预警或恶化信号，并用明确标记的宽概率区间表达具体原因仍不确定；最低覆盖
...
交接六部分计 100 分，并设置完整运行、必要证据、引用、可能性区间完整性、禁用绝对结论、可行动阶段和人工审核
