...
- 本地开发固定只保留一套服务：后端在独立终端运行带 `--reload` 的 Uvicorn，前端需要热更新时
  在另一个独立终端运行 Vite。不得让多个 Agent 或终端重复占用 8000 端口。
- Agent 的 Shell/Bash 工具调用不得使用 `Start-Process`、后台符号或同类方式托管 Uvicorn、Vite
  等长期运行进程。工具执行器可能持续等待子进程，导致命令已经成功但会话仍显示运行中。
- 修改后端代码后依赖 Uvicorn 自动重载，并通过健康接口确认恢复；不得为了加载代码反复强杀全部
  Python 进程。修改前端代码时依赖 Vite 热更新，不需要重启后端。
- 如果当前没有开发服务，Agent 应提示用户在独立终端启动，或使用可见的专用终端会话；启动、停止
  和重启都由同一个终端负责。不得把长期服务附着在一次性测试或验收命令中。
- 排查端口时必须同时检查监听 PID、父子进程关系和健康接口。同一时刻启动的虚拟环境 Python
  启动器及其基础解释器子进程视为一套服务，不得仅因出现两个 `python.exe` 就判断为端口冲突。
