...
| 引用与推理 | 30 | 引用存在、事实/风险引用、可能性区间完整性、风险证据多样性 |
| 结论边界 | 10 | 禁用绝对结论、未知项局部弃答、可接受风险阶段 |
| 人工交接 | 10 | 必须人工审核、行动、监测项和优先级 |

总分不能掩盖底线。自动通过还必须同时满足：完整运行、必要证据覆盖、引用完整、可能性区间合法、没有
...
上一版模型协议候选在同一评测集和数据快照上完成 6 案 × 2 轮：12/12 完整、自动门槛 12/12、人工
语义复核 12/12、最终发布门槛 12/12，平均分与最低分均为 100，六个案件的阶段和自动门槛均跨轮
一致。该结果保留为历史对照，不代表后续模型协议改造仍自动满足发布门槛。

切换到 DeepSeek Chat Completions、英文指令和 JSON Mode 后，已完成
`INV-83NN0001CD-MIXED-SIGNALS` 单案真实冒烟：报告完整、自动门槛通过、100 分。发布前仍需重新执行
6 案 × 2 轮完整评测和人工语义复核。
